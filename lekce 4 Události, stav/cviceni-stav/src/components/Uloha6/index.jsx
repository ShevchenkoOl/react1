...
const Ukol = ({ nazev }) => {
  ...
  return (
    <li className={isPushed ? "ukol ukol--splnen" : "ukol"}>
      <span className="ukol__nazev">{nazev}</span>
      {!isPushed ? <button className="ukol__akce" onClick={()=>setIsPushed(!isPushed)}>splnit</button> : null}
    </li>
  );
};
